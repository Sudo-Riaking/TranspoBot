...
from fastapi import FastAPI, HTTPException #Framework web rapide et moderne
from fastapi.staticfiles import StaticFiles #Pour servir une interface frontend (optionnel)
from fastapi.middleware.cors import CORSMiddleware #Middleware pour gérer les CORS (Cross-Origin Resource Sharing)"""
from pydantic import BaseModel #Pour définir des modèles de données (ex: requêtes API)'''
import mysql.connector     #Client MySQL pour Python"""
import os #Pour accéder aux variables d'environnement (config)'''
import re #Pour extraire le JSON de la réponse du LLM'''
import httpx #Client HTTP asynchrone pour appeler l'API du LLM'''
...
LLM_API_KEY  = os.getenv("OPENAI_API_KEY", "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")
...
def execute_query(sql: str): 
    conn = get_db()
    cursor = conn.cursor(dictionary=True) #Retourner les résultats sous forme de dictionnaires (colonnes comme clés)
    try:        #Exécuter la requête SQL et récupérer les résultats
        cursor.execute(sql)
        return cursor.fetchall() #Récupérer tous les résultats de la requête sous forme de liste de dictionnaires
    finally:
        cursor.close()
        conn.close()
